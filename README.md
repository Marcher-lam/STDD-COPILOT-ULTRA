...
[![Tests](https://img.shields.io/badge/tests-4378%2F4378%20passing-brightgreen.svg)](CONTRIBUTING.md)
...
| 命令模板 | 88 个（`/stdd:*` 斜杠命令） |
| Skill 模板 | 57 个 |
| 全部能力入口 | 91 个（88 Command + 57 Skill，去重后） |
| 测试套件 | **206** 套，**4378** 条用例，100% 通过 |
| 语句覆盖率 | **97.7%** |
| 分支覆盖率 | **93.2%** |
...
### Ultra 增强命令（Phase 2-4）

| CLI | 用途 |
|-----|------|
| `stdd builder create / list` | 创建自定义 Agent、Workflow、Skill |
| `stdd ui create / list` | 基于 DESIGN.md 设计令牌生成前端页面和组件 |
| `stdd modules search / install / list` | 模块市场：浏览、搜索、安装社区模块 |
| `stdd dashboard generate / open` | 静态 HTML 项目健康仪表板 |
| `stdd docs build / serve` | 从项目文档生成静态文档站点 |
| `stdd profile detect / set` | 规划深度自适应（按项目复杂度自动调节） |
| `stdd prfaq ignition / press-release / full` | Amazon Working Backwards PRFAQ 工作流 |
| `stdd codegraph inspect / query / update` | 代码知识图谱：代码关系、依赖分析 |
| `stdd iterate plan / execute / reflect` | Plan-Execute-Reflect 迭代循环 |
...
- Builder 引擎创建自定义 Agent、Workflow、Skill
- 模块市场（Modules Marketplace）支持社区扩展
- Dashboard + Docs Site 静态可视化
...
│   └── types/                      # JSDoc 类型定义
├── src/templates/
│   ├── commands/                   # 88 个斜杠命令模板（Markdown）
│   └── skills/stdd/                # 57 个 Skill 模板目录
...
| [docs/command-reference.md](./docs/command-reference.md) | 全部 88 个命令详细说明 |
