...
这份文档说明：如果你已经有一个老项目，想通过 STDD 来做修改或重构，应该怎么操作。
...
**预期产物**: `stdd/` 目录、`stdd/config.yaml`、项目上下文文件、PR 模板或 issue 模板。
...
- [ ] `stdd/config.yaml` 存在。
- [ ] 技术栈检测结果合理。
- [ ] 如果项目已有 `.github/` 文件，STDD 不应破坏已有 CI 配置。
...
**预期产物**: `stdd/changes/bugfix-*/proposal.md`、`tasks.md`、`specs/`。

**验证点**:

- [ ] proposal 包含 Steps to Reproduce。
- [ ] tasks 第一项是失败测试或 characterization test。
...
### B13 — 补充 characterization test

```bash
stdd apply --phase red --task TASK-001
```

**预期产物**: 先写出能暴露 bug 或锁定旧行为的测试。

**验证点**:

- [ ] RED 阶段测试应该失败。
- [ ] 失败原因对应支付回调重复处理。
- [ ] 如果老项目没有测试命令，STDD 应阻止继续并要求配置测试命令。

### B14 — 最小修复

```bash
stdd apply --phase green --task TASK-001
```
...
```bash
stdd apply --phase refactor --task TASK-001
```
...
**预期产物**: Golden Packet 风格修复上下文。

**验证点**:

- [ ] 包含 proposal、tasks、失败/通过证据。
- [ ] 便于 code review 或交给另一个 AI/工程师复核。
...
- [ ] specs 不应写死具体文件路径或内部类名。
- [ ] specs 关注行为：选择退款状态后列表筛选正确。
...
```bash
stdd apply order-refund-filter --task TASK-001
...
stdd apply refactor-order-service --phase red --task TASK-001
```
...
- [ ] 复杂度下降或职责更清晰。
- [ ] 测试覆盖不下降。
- [ ] Constitution 无 blocking violation。

---

# Part V: 处理测试不足或无测试的老项目

> **场景**: 很多老项目没有测试。STDD 不应该盲目修改，而应先建立最低保护网。
...
```bash
stdd issue "订单状态流转缺少回归测试，重构前需要补 characterization tests" --title "补订单状态流转测试" --severity medium
```
...
- [ ] 检测到 apps/packages。
- [ ] package name、root、package.json 路径正确。
...
- [ ] change metadata 记录 workspace。
- [ ] specs/tasks 针对 `apps/admin`。
...
- [ ] 类型准确：bugfix 用 `fix:`，功能用 `feat:`，重构用 `refactor:` 或明确 scope。
...
| # | 场景 | 命令 | 状态 | 备注 |
|---|------|------|------|------|
| B01 | 工作区状态 | `git status --short --branch` | | |
| B02 | 项目元信息 | `package.json` / `README.md` | | |
| B03 | 原始质量基线 | `npm test` / `lint` / `build` | | |
| B04 | 项目探索 | `stdd explore` | | |
| B05 | 初始化 | `stdd init --yes` | | |
| B06 | 深度诊断 | `stdd doctor --deep` | | |
| B07 | 测试命令配置 | `stdd/config.yaml` | | |
| B08 | 代码记忆 | `stdd memory scan` | | |
| B09 | 风格学习 | `stdd learn scan` | | |
| B10 | 上下文查看 | `stdd context` | | |
| B11-B17 | Bug 修复 | `stdd issue/apply/verify` | | |
| B18-B22 | 小功能修改 | `stdd ff/apply/user-test` | | |
| B23-B29 | 安全重构 | `stdd explore/metrics/new/apply` | | |
| B30-B33 | 测试补强 | `stdd tdd-init` | | |
| B34-B37 | Monorepo | `stdd workspace` | | |
| B38-B41 | 高风险治理 | `roles/constitution` | | |
| B42-B44 | 断点续传 | `stdd progress/continue` | | |
| B45-B48 | 交付归档 | `verify/guard/archive/commit` | | |
