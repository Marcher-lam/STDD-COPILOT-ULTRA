...
- **模板系统**：20 个命令模板 + 38 个技能模板 = 58 个定义入口（去重后 45 个唯一）
- **CLI 工具**：53 个顶级命令，终端直接执行
