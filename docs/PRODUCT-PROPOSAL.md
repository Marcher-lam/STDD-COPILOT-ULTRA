...
- **模板系统**：88 个命令模板 + 57 个技能模板 = 145 个定义入口（去重后 91 个唯一）
...
├── src/
│   ├── cli/commands/            # 88 个命令实现文件
│   ├── cli/registry/            # 命令注册与动态加载
│   ├── utils/                   # 21 个工具模块
│   ├── runtime/                 # 浏览器驱动、Agent 引擎
│   └── templates/
│       ├── skills/              # 57 个 Skill 定义目录
│       └── commands/            # 20 个斜杠命令模板
...
### 15.2 完整斜杠命令列表（145 个定义入口，去重后 91 个唯一入口）
