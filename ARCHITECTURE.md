...
`stdd graph run` 是用户可用的 CLI 编排入口，位于 `src/cli/commands/graph-run.js`。它通过 `DynamicGraphRouter` 编译 `feature`、`hotfix`、`research` 等意图 DAG，并把节点映射到已实现 CLI 能力：`ff`、`spec`、`apply`、`verify`、`archive` 等。
