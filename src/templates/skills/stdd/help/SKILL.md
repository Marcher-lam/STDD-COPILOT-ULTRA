...
id: stdd.help
command: /stdd:help
description: 根据当前状态提供上下文感知帮助和下一步建议
version: "2.0"
...
## Purpose
根据当前状态提供上下文感知帮助和下一步建议。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:help 对应能力时。
...
## Workflow
- 检测 stdd/config、active change、status 和 workspace。
- 按当前阶段显示命令、示例和下一步。
- 可聚焦 tdd、graph、constitution、workspace 等主题。
...
stdd recommend
```
支持 CLI 与 `/stdd:help` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
