...
## Purpose
聚合变更产物生成最终交付文档。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:final-doc 对应能力时。
...
## Workflow
- 读取 proposal、specs、design、tasks、evidence、metrics 和 archive 状态。
- 生成变更级最终文档，说明需求、设计、实现、测试和风险。
- 缺失证据必须标注，不伪造。
...
```
支持 CLI 与 `/stdd:final-doc` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
