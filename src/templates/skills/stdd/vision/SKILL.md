...
## Purpose
维护项目愿景、北极星原则和成功指标。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:vision 对应能力时。
...
## Workflow
- 梳理问题、用户、价值主张、技术北极星、非目标和指标。
- 对齐当前 specs、roadmap 和 Constitution。
- greenfield 创建愿景；brownfield 从 README/代码现实反推并等待确认。
...
stdd context --export
```
支持 CLI 与 `/stdd:vision` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
