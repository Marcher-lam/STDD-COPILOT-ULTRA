...
id: stdd.plan
command: /stdd:plan
description: 从规格生成技术设计和原子 TDD tasks
version: "2.0"
category: lifecycle
phase: planning
read_only: false
...
## Purpose
从规格生成技术设计和原子 TDD tasks。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:plan 对应能力时。
...
## Workflow
- 读取 specs、proposal、项目技术栈和 workspace 约束。
- 生成 technical design、ADR 摘要、风险、测试策略和文件级影响。
- 拆分 5-6 个左右原子任务，每个任务可独立 Red-Green-Refactor。
- 更新 tasks.md、.status.yaml 与 planning evidence。
...
stdd plan --resume
```
支持 CLI 与 `/stdd:plan` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
