...
description: 人类确认门，冻结需求范围后进入规格阶段
version: "2.0"
category: lifecycle
phase: confirmation
read_only: false
...
## Purpose
人类确认门，冻结需求范围后进入规格阶段。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:confirm 对应能力时。
...
## Workflow
- 展示 proposal 摘要、范围、验收标准、风险和未决假设。
- 要求人类明确确认；自然语言自主编排不得跳过此门。
- 确认后更新 .status.yaml；否决则回到 propose/clarify。
...
```
支持 CLI 与 `/stdd:confirm` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
