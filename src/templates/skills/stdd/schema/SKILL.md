...
id: stdd.schema
command: /stdd:schema
description: 生成和验证 JSON Schema、Zod 与类型约束
version: "2.0"
category: spec-first
phase: specification
read_only: false
...
## Purpose
生成和验证 JSON Schema、Zod 与类型约束。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:schema 对应能力时。
...
## Workflow
- 从 specs 或显式字段定义生成 JSON Schema/Zod。
- 标注 required、format、min/max、enum 与错误消息。
- 可 validate/fork 既有 schema，避免类型漂移。
- 将 schema evidence 链接回 change。

## CLI Runtime
```bash
stdd schema create <name>
stdd schema validate schemas/spec-driven/
stdd schema fork <src> <name>
```
支持 CLI 与 `/stdd:schema` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
