...
id: stdd.spec
command: /stdd:spec
description: 从已确认需求生成可测试 BDD delta specs
version: "2.0"
category: lifecycle
phase: specification
read_only: false
...
## Purpose
从已确认需求生成可测试 BDD delta specs。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:spec 对应能力时。
...
## Workflow
- 读取已确认 proposal，不把文件路径或实现类名写入行为规格。
- 生成 ADDED/MODIFIED/REMOVED delta specs 与 Given/When/Then 场景。
- 覆盖边界条件、错误路径和兼容性要求。
- 运行 validate 或提示运行 validate，再进入 plan。
...
```
支持 CLI 与 `/stdd:spec` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
