...
## Purpose
保存和检索跨会话语义记忆与决策。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:memory 对应能力时。
...
## Workflow
- 保存高价值决策、模式、失败修复和架构约束。
- 用 topic、change-id、workspace 元数据索引。
- 搜索时返回相似度和来源路径。
- 禁止保存 secrets、token、凭据。
...
stdd memory stats
```
支持 CLI 与 `/stdd:memory` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
