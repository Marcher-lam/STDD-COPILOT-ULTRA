...
description: 为失败任务生成 Golden Packet 风格修复上下文
version: "2.0"
category: evidence
phase: verification
read_only: false
...
## Purpose
为失败任务生成 Golden Packet 风格修复上下文。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:fix-packet 对应能力时。
...
## Workflow
- 定位失败任务、最近 test output、相关 specs/design/tasks。
- 收集源码片段、命令、环境、失败次数和假设。
- 生成 Markdown 与 JSON 双格式 packet。
- 供 repair intent、apply retry 或人工诊断使用。
...
```
支持 CLI 与 `/stdd:fix-packet` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
