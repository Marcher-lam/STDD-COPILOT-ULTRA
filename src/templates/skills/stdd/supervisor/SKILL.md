...
description: 协调多 Agent 任务委派、同步和失败升级
version: "2.0"
...
## Purpose
协调多 Agent 任务委派、同步和失败升级。这是 STDD Copilot 的 Spec-First + TDD CLI skill，服务 Skill Graph 编排、Constitution gate、evidence 留痕和 workspace 作用域。

## When to Use
- 需要执行 /stdd:supervisor 对应能力时。
...
## Workflow
- 建立 agent 角色、任务队列、依赖和同步点。
- 分派独立任务，收集状态、阻塞和 evidence。
- 失败重试后升级给人类或 fix-packet。
- 支持 progress resume 与 graph history。

## CLI Runtime
```bash
stdd runtime agent start "topic" --rounds 3
stdd runtime agent next
stdd runtime agent stop
```
支持 CLI 与 `/stdd:supervisor` 双入口；在 monorepo 中优先传入 `--workspace <path-or-package>` 并把证据写入对应作用域。
